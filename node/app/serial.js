...
function inserirRegistro(cel) {
    request = new Request("INSERT into Temperatura values (@cod_arduino, @temperatura, CURRENT_TIMESTAMP);", function (err) {
        if (err) {
            console.log(err);
        }
    });
    request.addParameter('cod_arduino', TYPES.Int, 2);
    request.addParameter('temperatura', TYPES.Float, cel);
